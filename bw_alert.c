...
void send_alert(const char *ip_address, const char *host_address) {
    CURL *curl;
    CURLcode res;
    char url[256];
    char json_data[512];
    ...
    snprintf(json_data, sizeof(json_data),
             "{ \"hostName\": \"%s\", \"state\": \"red\" }", host_address);
...
void monitor_bandwidth(const char *interface, int interval, double threshold, char *ip_list, const char *host_address) {
    long long rx1, rx2;
    double rx_mbps;
    ...
    while (1) {
        ...
        printf("Interface: %s | RX Bandwidth: %.2f Mbps\n", interface, rx_mbps);

        // If bandwidth exceeds threshold, send alerts
        if (rx_mbps > threshold) {
            printf("Threshold exceeded! Sending alerts...\n");
            for (int i = 0; i < ip_count; i++) {
                send_alert(ip_addresses[i], host_address);
            }
        }
...
int main(int argc, char *argv[]) {
    if (argc < 6) {
        printf("Usage: %s <interface> <interval_seconds> <rx_threshold_mbps> <ip_list> <host_address>\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    strncpy(ip_list, argv[4], sizeof(ip_list) - 1);
    ip_list[sizeof(ip_list) - 1] = '\0';

    strncpy(host_address, argv[5], sizeof(host_address) - 1);
    host_address[sizeof(host_address) - 1] = '\0';

    monitor_bandwidth(interface, interval, threshold, ip_list, host_address);
